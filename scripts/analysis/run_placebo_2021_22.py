"""Placebo sazonal com dados reais de 2021/22 na pausa FIFA de novembro.
...
coorte que seria convocada em 2022 já apresentava uma mudança diferencial de
utilização em uma temporada sem Copa no meio do calendário.
...
RAW = ROOT / "data" / "raw" / "transfermarkt" / "dcaribou_2021_22"
INTERIM = ROOT / "data" / "interim"
PROCESSED = ROOT / "data" / "processed"
REPORTS = ROOT / "reports" / "placebo_2021_22"
CONFIG_PATH = ROOT / "config" / "placebo_2021_22.yml"
...
def read_games() -> pd.DataFrame:
    ...
    games = games.loc[
        games["season"].eq(2021) & games["competition_id"].isin(BIG5)
    ].copy()
    if len(games) != 1826:
        raise RuntimeError(f"Cobertura Big Five 2021/22 inválida: {len(games)} jogos, esperado 1826")
...
def quality_table(grid: pd.DataFrame, schedule: pd.DataFrame, risk: pd.DataFrame, weights: pd.DataFrame, results: pd.DataFrame, config: dict[str, object]) -> pd.DataFrame:
    ...
    rows = [
        ...
        ("window_uses_2021_november_pause", pd.Timestamp(config["baseline_cutoff"]).date() == pd.Timestamp("2021-11-07").date() and pd.Timestamp(config["post_start"]).date() == pd.Timestamp("2021-11-19").date(), True),
        ...
        ("weighting_selection_gates_passed", bool(selected_candidate["passes_selection_gates"]), bool(selected_candidate["passes_selection_gates"])),
    ]
...
def write_manifest() -> None:
    ...
    for path in sorted(REPORTS.glob("*.csv")):
        ...
        records.append({"relative_path": path.name, "rows": rows, "bytes": path.stat().st_size, "sha256": sha256(path), "generated_by": "scripts/analysis/run_placebo_2021_22.py"})
...
def write_readme(flow: pd.DataFrame, diagnostics: pd.DataFrame, results: pd.DataFrame) -> None:
    ...
    text = f"""# Placebo sazonal 2021/22 — pausa FIFA de novembro
...
def main() -> None:
    ...
    write_manifest()
    print("Placebo 2021/22 concluído. Consulte reports/placebo_2021_22/README.md.")
